...
**Defaults (Kconfig-overridable, seeded to NVS):** relays GPIO 4/5 active-high, buttons GPIO 6/7 pull-up, I2C SDA 8/SCL 9, power-on `restore`, interval 30s; **set default room = `phong-khach`** (amend design doc's `livingroom` — matches roadmap examples). `node_id` = `esp32s3-`+last-6-hex STA MAC.
...
|---|---|
| Node appears in web UI ≤10s after flash, zero server-side action | Board on COM4, broker up → watch dashboard; node auto-appears via discovery upsert (expect 4–7s). |
